...
interface ClassicAppProps {
  onSwitchToMarathonUI?: () => void;
}

export const ClassicApp: React.FC<ClassicAppProps> = ({ onSwitchToMarathonUI }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans flex flex-col selection:bg-emerald-500 selection:text-white">
      {/* 留档版本提示横幅 */}
      <div className="bg-gradient-to-r from-amber-950/80 via-slate-900 to-amber-950/80 border-b border-amber-500/30 px-4 py-2 text-xs flex items-center justify-between">
        <div className="flex items-center gap-2 text-amber-200">
          <span className="px-2 py-0.5 rounded bg-amber-500/20 text-amber-300 font-mono text-[10px] font-bold border border-amber-500/30">
            CLASSIC ARCHIVE // 经典留档版本
          </span>
          <span>当前显示为用户留档保存的经典金融终端界面。所有实时行情、爬虫及预警计算 100% 完整运行。</span>
        </div>
        {onSwitchToMarathonUI && (
          <button
            onClick={onSwitchToMarathonUI}
            className="px-3 py-1 rounded bg-[#D4FF00] hover:bg-[#b8e000] text-black font-bold text-xs flex items-center gap-1.5 transition-all shadow-sm active:scale-95 cursor-pointer"
          >
            <span>切换至全新 MARATHON 战术视觉界面 →</span>
          </button>
        )}
      </div>
      ...
      <footer className="border-t border-slate-900 bg-slate-950 py-4 text-center text-xs text-slate-400">
        <div className="max-w-7xl mx-auto px-4 flex flex-col sm:flex-row items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            <span>0成本生猪高频监控雷达 (Pork Radar Dynamic Free) · 持续高频推流测算中 · [经典留档版]</span>
          </div>
          <div className="text-[11px] text-slate-400">
            数据源：大连商品交易所(DCE)生猪期货 + 搜猪网/农业农村部现货 + 华储网官方储备肉公告 + 东方财富/新浪A股实时行情
          </div>
        </div>
      </footer>
