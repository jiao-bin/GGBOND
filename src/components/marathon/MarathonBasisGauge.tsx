  ...
  return (
    <div className="rounded border border-white/14 marathon-card-translucent marathon-card-invert p-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 pb-3 border-b border-white/10">
        ...
        <div className="flex items-center gap-2">
          <span className={`px-2 py-0.5 rounded text-xs font-mono font-bold border ${zoneColor} bg-black preserve-color`}>
            {zoneBadge}
          </span>
          <span className="text-xs text-slate-400 font-mono">
            基差(现-期): <strong className="text-white">{basisTon} 元/吨</strong>
          </span>
        </div>
      </div>
      ...
      <div className="mt-4 pt-2">
        <div className="relative mb-2">
          {/* 刻度指示滑块 - 带有白底与白箭头，与下方黑色标尺轨道截然分明 */}
          <div
            className="absolute -top-4 transition-all duration-300 z-10 flex flex-col items-center -translate-x-1/2 pointer-events-none select-none"
            style={{ left: `${pointerPct}%` }}
          >
            <div className="px-2 py-0.5 rounded bg-white text-black text-xs font-mono font-black shadow-md border border-slate-300 gauge-pointer flex items-center justify-center">
          ...
          {/* 标尺轨道 */}
          <div className="h-6 w-full flex rounded border border-white/15 bg-black overflow-hidden font-mono text-[11px] font-bold">
            {/* 深度贴水区 (-15% ~ -5%) */}
            <div className="w-[25%] bg-emerald-950/40 border-r border-white/10 flex items-center justify-center text-emerald-400 preserve-color">
              贴水 &lt;-5%
            </div>
            {/* 平水区 (-5% ~ +5%) */}
            <div className="w-[25%] bg-white/[0.04] border-r border-white/10 flex items-center justify-center text-slate-300 preserve-color">
              平水 (-5%~+5%)
            </div>
            {/* 温和升水区 (+5% ~ +15%) */}
            <div className="w-[25%] bg-[#2000E0]/20 border-r border-white/10 flex items-center justify-center text-[#D4FF00] preserve-color">
              温和升水 (+5%~+15%)
            </div>
            {/* 极端升水区 (+15% ~ +25%) */}
            <div className="w-[25%] bg-red-950/40 flex items-center justify-center text-[#FF4600] preserve-color">
              极高升水 &gt;+15%
            </div>
          </div>
        </div>
      ...
      {/* 产业博弈解读与交易策略建议 */}
      <div className="mt-3.5 pt-3 border-t border-white/10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs">
        <div className="flex items-center gap-2">
